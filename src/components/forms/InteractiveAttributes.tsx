...
export function InteractiveAttributes({
  type,
  title,
  description,
  placeholder,
  ...
  const hoverColorClass = isEntrada
    ? "hover:bg-[#008f94]"
    : "hover:bg-[#008141]";
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "bg-white border-2 border-slate-200 rounded-xl p-6 transition-all duration-300 hover:shadow-md h-[450px] flex flex-col",
      ...
      >
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button
            ...
            onClick={handleAdd}
            className={cn(
              "px-4 py-2 text-white font-semibold rounded-lg flex items-center gap-2 transition-all transform active:scale-95 text-sm",
              themeColorClass,
              hoverColorClass,
            )}
          >
            <MsIcon name="add" size={18} /> Inserir
          </button>
        </div>
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <motion.div
                  ...
                  className={cn(
                    "flex items-center justify-between p-3 border border-slate-200 rounded-lg group",
                    lightBgClass,
                  )}
                >
