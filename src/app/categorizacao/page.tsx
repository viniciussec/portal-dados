...
export default function CategorizacaoPage() {
  ...
  const SUBCATEGORIAS = {
    amplo: [
      ...
      {
        id: "A02",
        label: "A02 - Informação sobre funcionamento governamental",
      },
      {
        id: "A03",
        label: "A03 - Informações declaradas públicas pelos órgãos",
      },
      {
        id: "A04",
        label: "A04 - Situação de regularidade de Pessoas Jurídicas",
      },
  ...
  const validateFields = () => {
    ...
    if (formData.contatoTecnico && !isValidEmail(formData.contatoTecnico))
      missing.push({
        key: "contatoTecnico",
        label: "Contato Técnico (E-mail inválido)",
      });
    return missing;
  };
  ...
  const buildPreviewContent = () => {
    const subLabel = formData.subcategoria
      ? SUBCATEGORIAS[formData.categoria as keyof typeof SUBCATEGORIAS]?.find(
          (s) => s.id === formData.subcategoria,
        )?.label || formData.subcategoria
      : "";
      ...
      <div className="space-y-4">
        <div className="text-center pb-4 border-b border-slate-200">
          ...
          <p className="font-extrabold text-slate-800 text-base">
            Ficha de Categorização de Dados
          </p>
        ...
        >
          ...
          <Row
            label="Data da Categorização"
            value={formatDate(formData.dataCat)}
          />
        </Section>

        <Section
          title="Tipo e Detalhes da Informação"
          colorClass="bg-blue-50 text-blue-800"
        >
          <Row
            label="Tipo de Informação"
            value={TIPO[formData.tipoInfo] || formData.tipoInfo}
          />
        ...
        <Section
          ...
          colorClass={
            formData.categoria === "amplo"
              ? "bg-green-50 text-green-800"
              : formData.categoria === "restrito"
        ...
        >
          <Row
            label="Categoria"
            value={formData.categoria ? formData.categoria.toUpperCase() : ""}
          />
          <Row label="Subcategoria" value={subLabel} />
          {formData.regrasCompartilhamento && (
            <Row
              label="Regras Complementares"
              value={formData.regrasCompartilhamento}
            />
          )}
          {formData.fundamentacaoEspecifica && (
            <Row
              label="Fundamentação Específica"
              value={formData.fundamentacaoEspecifica}
            />
          )}
        </Section>
      </div>
    ...
    const pdfData = [
      ...
      },
      {
        ...
        content: [
          ...
          ...(formData.regrasCompartilhamento
            ? [`Regras Complementares: ${formData.regrasCompartilhamento}`]
            : []),
          ...(formData.fundamentacaoEspecifica
            ? [
                `Fundamentação para Categorização Específica: ${formData.fundamentacaoEspecifica}`,
              ]
            : []),
        ],
      },
    ...
    <>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden relative">
          ...
          <div className="bg-slate-50 border-b border-slate-200 p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-slate-500 font-medium">
                  <b>
                    Subcomitê de Governança de Dados do Governo do Estado do
                    Ceará
                  </b>
                </p>
              </div>
          ...
          <div className="p-8 space-y-12">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 text-amber-900 shadow-sm">
              ...
              <ul className="space-y-2 text-sm leading-relaxed">
                <li>
                  Essa ficha serve para categorizar dados que podem ser
                  compartilhados com outros órgãos e entidades. Ela deve ser
                  preenchida pelo órgão gestor do dado. Após o preenchimento
                  completo dela, você deve <strong>exportar para PDF</strong> e
                  enviá-la para o{" "}
                  <strong>Subcomitê de Governança de Dados</strong>.{" "}
                </li>
                <li>
                  Resolução nº 01/2026, Art. 3.5 e Decreto nº 37.059/2026.
                </li>
                <li>
                  <strong>Base legal:</strong> Resolução nº 01/2026, Art. 3.5 e
                  Decreto nº 37.059/2026.{" "}
                </li>
              </ul>
            </div>
            ...
            <section className="bg-white border text-sm border-slate-200 rounded-xl p-6 shadow-sm">
              <h2 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2 border-b-2 border-slate-100 pb-3">
                <span className="text-[#009a4d]">📋</span> Identificação do
                Gestor de Dados
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block font-bold text-slate-700 mb-2">
                    Órgão/Entidade Gestora de Dados{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    className="w-full p-2.5 border-2 border-slate-200 bg-slate-50 rounded-lg focus:border-[#009a4d] focus:bg-white focus:ring-4 focus:ring-green-50 transition-all outline-none"
                    value={formData.orgaoGestor}
                    onChange={(e) =>
                      setFormData({ ...formData, orgaoGestor: e.target.value })
                    }
                  />
                  <p className="text-xs text-slate-500 mt-2">
                    Nome completo do órgão responsável pela governança dos dados
                  </p>
                </div>
                <div>
                  <label className="block font-bold text-slate-700 mb-2">
                  ...
                  <input
                    type="text"
                    className="w-full p-2.5 border-2 border-slate-200 bg-slate-50 rounded-lg focus:border-[#009a4d] focus:bg-white focus:ring-4 focus:ring-green-50 transition-all outline-none"
                    value={formData.respTecnico}
                    onChange={(e) =>
                      setFormData({ ...formData, respTecnico: e.target.value })
                    }
                  />
                </div>
                <div>
                  <label className="block font-bold text-slate-700 mb-2">
                    Contato Técnico (E-mail){" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="email"
                    className={cn(
                      "w-full p-2.5 border-2 bg-slate-50 rounded-lg focus:bg-white focus:ring-4 transition-all outline-none",
                      emailError
                        ? "border-red-400 focus:border-red-500 focus:ring-red-50"
                        : "border-slate-200 focus:border-[#009a4d] focus:ring-green-50",
                    )}
                  ...
                  {!emailError && (
                    <p className="text-xs text-slate-500 mt-1">
                      E-mail para esclarecimentos técnicos sobre a categorização
                    </p>
                  )}
                </div>
                <div>
                  <label className="block font-bold text-slate-700 mb-2">
                    Data da Categorização{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="date"
                    className="w-full p-2.5 border-2 border-slate-200 bg-slate-50 rounded-lg focus:border-[#009a4d] focus:bg-white focus:ring-4 focus:ring-green-50 transition-all outline-none"
                    value={formData.dataCat}
                    onChange={(e) =>
                      setFormData({ ...formData, dataCat: e.target.value })
                    }
                  />
                </div>
              </div>
            ...
            <section className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm text-sm">
              <h2 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2 border-b-2 border-slate-100 pb-3">
                <span className="text-[#009a4d]">📑</span> Escolha o tipo de
                informação que você está categorizando
              </h2>
              ...
              <div className="bg-slate-50 border border-slate-200 text-slate-700 p-4 rounded-lg flex items-start gap-3 mb-6">
                ...
                <p>
                  ...
                  <br />
                  <br />
                  <strong>Consulta de Registro Único:</strong> Pesquisa que
                  retorna informações sobre um indivíduo ou objeto específico.
                  Exemplos: consultar os dados biográficos de um CPF ou detalhes
                  de um veículo a partir da placa dele.
                  <br />
                  <br />
                  <strong>Conjunto de Informações:</strong> Pesquisa baseada em
                  filtros (parâmetros) que retorna uma lista de resultados.
                  Exemplos: lista dos beneficiários do Vale Gás ou lista de
                  veículos com licenciamento atrasado em um município.
                </p>
              </div>
              ...
              {formData.tipoInfo === "conjunto" && (
                <div className="mt-4 bg-green-50 text-green-800 p-4 rounded-lg border border-green-200">
                  ...
                  Pesquisa baseada em filtros (parâmetros) que retorna uma lista
                  de resultados. Exemplos: lista dos beneficiários do Vale Gás
                  ou lista de veículos com licenciamento atrasado em um
                  município
                </div>
              )}
            </section>
            ...
            <section className="bg-white border text-sm border-slate-200 rounded-xl p-6 shadow-sm">
              <h2 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2 border-b-2 border-slate-100 pb-3">
                <span className="text-[#009a4d]">📝</span> Informe detalhes da
                informação a ser categorizada
              </h2>

              <div className="space-y-6">
                <div>
                  <label className="block font-bold text-slate-700 mb-2">
                    Título do Dado <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    className="w-full p-2.5 border-2 border-slate-200 bg-slate-50 rounded-lg focus:border-[#009a4d] focus:bg-white focus:ring-4 focus:ring-green-50 transition-all outline-none"
                    value={formData.tituloDado}
                    onChange={(e) =>
                      setFormData({ ...formData, tituloDado: e.target.value })
                    }
                  />
                  <p className="text-xs text-slate-500 mt-2">
                    Título claro e descritivo que caracterize os dados
                    categorizados
                  </p>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    className="w-full p-2.5 border-2 border-slate-200 bg-slate-50 rounded-lg focus:border-[#009a4d] focus:bg-white focus:ring-4 focus:ring-green-50 transition-all outline-none min-h-30"
                    value={formData.descricaoDado}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        descricaoDado: e.target.value,
                      })
                    }
                  />
                  <p className="text-xs text-slate-500 mt-2">
                    Definição detalhada do dado ou informação categorizada
                  </p>
                </div>
            ...
            <section className="bg-white border text-sm border-slate-200 rounded-xl p-6 shadow-sm">
              <h2 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2 border-b-2 border-slate-100 pb-3">
                <span className="text-[#009a4d]">🔐</span> Escolha a categoria
                de compartilhamento
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div
                  className={cn(
                    "border-2 rounded-xl p-5 cursor-pointer transition-all hover:-translate-y-1 relative overflow-hidden",
                    formData.categoria === "amplo"
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-full bg-[#009a4d] flex items-center justify-center text-white text-xl">
                      🟢
                    </div>
                    <div className="font-bold text-[#009a4d] text-lg">
                      AMPLO
                    </div>
                  </div>
                  <p className="text-sm text-slate-600 mb-3">
                    Dados públicos sem restrição de acesso, com divulgação
                    garantida.
                  </p>
                  <div className="text-xs bg-black/5 text-slate-700 p-2 rounded">
                    Ex: Diário Oficial, Dados abertos, Contratos
                  </div>
                </div>
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-full bg-amber-500 flex items-center justify-center text-white text-xl">
                      🟡
                    </div>
                  ...
                  <p className="text-sm text-slate-600 mb-3">
                    Compartilhamento condicionado por sigilo legal ou segurança.
                  </p>
                  <div className="text-xs bg-black/5 text-slate-700 p-2 rounded">
                    Ex: Dados tributários, Dados protegidos por lei
                  </div>
                </div>
                ...
                >
                  ...
                  <p className="text-sm text-slate-600 mb-3">
                    Nível máximo de controle. Acesso avaliado caso a caso.
                  </p>
                  <div className="text-xs bg-black/5 text-slate-700 p-2 rounded">
                    Ex: Dados sensíveis, Inquéritos judiciais
                  </div>
                </div>
              </div>
              ...
              {formData.categoria && (
                <div className="mt-8 p-6 bg-slate-50 border border-slate-200 rounded-xl animate-in fade-in duration-300 space-y-6">
                  <div>
                    <label className="block font-bold text-slate-700 mb-2">
                      Subcategoria ({formData.categoria.toUpperCase()}){" "}
                      <span className="text-red-500">*</span>
                    </label>
                    <select
                    ...
                    >
                      ...
                      {SUBCATEGORIAS[
                        formData.categoria as keyof typeof SUBCATEGORIAS
                      ].map((sub) => (
                  ...
                  <div>
                    <label className="block font-bold text-slate-700 mb-2">
                      Regras Complementares de Compartilhamento
                    </label>
                    <p className="text-xs text-slate-500 mt-1">
                      {formData.categoria !== "especifico" && (
                        <span>
                          Regras adicionais para compartilhamento (opcional se
                          categoria não for Específico)
                        </span>
                      )}
                      {formData.categoria === "especifico" && (
                        <span>
                          Você categorizou o dado como compartilhamento{" "}
                          <strong>específico</strong>. <br /> Preencha quais são
                          as <strong>regras</strong> de acesso a esse dado,
                          esclarecendo quais são os órgãos e entidades que têm
                          direito a acessá-lo.
                        </span>
                      )}
                    </p>
                    <textarea
                      className="w-full p-2.5 border-2 border-slate-200 bg-white rounded-lg focus:border-[#009a4d] focus:ring-4 focus:ring-green-50 transition-all outline-none min-h-20"
                      value={formData.regrasCompartilhamento}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          regrasCompartilhamento: e.target.value,
                        })
                      }
                    />
                  </div>
                  {formData.categoria === "especifico" && (
                    <div>
                      <label className="block font-bold text-slate-700 mb-2">
                        Fundamentação para Categorização Específica
                      </label>
                      <p className="text-xs text-slate-500 mt-1">
                        Já que você categorizou o dado como compartilhamento{" "}
                        <strong>específico</strong>, justifique a necessidade de
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            fundamentacaoEspecifica: e.target.value,
                          })
                        }
                      />
                    </div>
          ...
          {/* Footer Action */}
          <div className="bg-slate-50 border-t border-slate-200 p-6 sm:p-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <button
